...
# operating system:  Linux 4.13.0-45-generic x86_64 [ELF] (4.13.0)
# libc:	(6.23.0)
# binutils:	(226)
...
SRCS = about.c help.c stats.c viewer.c xtar.c
OBJS = $(SRCS:.c=.o)
...
install:: xtar
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTPGMFLAGS)  xtar $(DESTDIR)$(BINDIR)/xtar

all:: xtar.$(MANNEWSUFFIX)

xtar.$(MANNEWSUFFIX): xtar.$(MANSRCSUFFIX)
	$(RM) $@
	if test -z "$(PREPROCESSMANPAGES)" ; then \
	   cd `dirname xtar` && \
	   $(LN) `basename xtar.$(MANSRCSUFFIX)` `basename $@`; \
	else \
	      	     $(RAWCPP)  $(MANDEFS) $(EXTRAMANDEFS) \
	     < xtar.$(MANSRCSUFFIX) | sed -e '/^#  *[0-9][0-9]*  *.*$$/d' 			-e '/^#line  *[0-9][0-9]*  *.*$$/d' 			-e '/^[ 	]*XCOMM$$/s/XCOMM/#/' 			-e '/^[ 	]*XCOMM[^a-zA-Z0-9_]/s/XCOMM/#/' 			-e '/^[ 	]*XHASH/s/XHASH/#/' 			-e '/\@\@$$/s/\@\@$$/\\/' >$@; \
	fi

cleandir::
	$(RM) xtar.$(MANNEWSUFFIX)

install.man:: xtar.$(MANNEWSUFFIX)
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTMANFLAGS) xtar.$(MANNEWSUFFIX) $(DESTDIR)$(MANDIR)/xtar.$(MANSUFFIX)
...
install:: XTar.ad
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTAPPFLAGS) XTar.ad $(DESTDIR)$(XAPPLOADDIR)/XTar
